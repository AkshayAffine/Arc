...
DROP TABLE FINAL_MASTER_TABLE

SELECT A.*, ------SALES
G.UNIT_COST,G.FULL_PRICE, ----ITEMS
F.PROMO_PRICE,F.PROMO_DESC, -----PROMO
E.PRICE_POINT,E.KEY_NUM AS MD_TAKEN_flag,E.MD_TAKEN_PERIOD,---MD_TAKEN
C.ATTRIBUTE1,C.ATTRIBUTE3,C.ATTRIBUTE3_NUMBER, -----items_cda
B.*,----hierarchy
...
ON coalesce(A.MERCHANDISE_KEY,D.merchandise_key)=B.HIERARCHY7_KEY

LEFT JOIN 
        (SELECT * FROM [dbo].[items_cda] WHERE LOCATION_KEY = 'AA')C   ------------------ITEMS_CDA-------------------
ON coalesce(A.MERCHANDISE_KEY,D.MERCHANDISE_KEY)=C.MERCHANDISE_KEY


left join 
(SELECT *,DATEDIFF(WEEK,CRT_EFFECTIVE_DATE,END_DATE) AS MD_TAKEN_PERIOD
 FROM MD_TAKEN_FINAL)
 e  ---------------MD_TAKEN--------------------
...
ON coalesce(A.MERCHANDISE_KEY,D.MERCHANDISE_KEY)=G.MERCHANDISE_KEY
