...
---,FIRST_VALUE(coalesce(EOP_DC_Inventory_Units,0)+coalesce(EOP_DC_On_Order_Units,0)+coalesce(EOP_INVENTORY_UNITS,0)+coalesce(EOP_ON_ORDER_UNITS,0)+coalesce(Net_SALES_UNITS,0)) over(partition by line order by fiscal_year ,fiscal_week)
 --as First_wk_inv
--,coalesce(EOP_DC_Inventory_Units,0)+coalesce(EOP_INVENTORY_UNITS,0)+coalesce(EOP_ON_ORDER_UNITS,0) as Wkly_total_inv
--,case when ((coalesce(lag(inventory),inventory)- Net_SALES_UNITS) - inventory) < 0 then (inventory - (coalesce(lag(inventory),inventory)- Net_SALES_UNITS)) else 0 end as Expected_tot
,FIRST_VALUE(inventory) over(partition by line order by fiscal_year ,fiscal_week) 
+ sum(case when ((coalesce(lag_inv,inventory)- Net_SALES_UNITS) - inventory) < 0 
then (inventory - (coalesce(lag_inv,inventory)- Net_SALES_UNITS)) else 0 end)
over(partition by line) as First_wk_inv
...
from
(select * ,lag(inventory) over(partition by line order by fiscal_year ,fiscal_week) as lag_inv 
from [dbo].[final_master_AD_2]) a 
where FISCAL_YEAR is not null and FISCAL_WEEK is not null
--where line=1442235
;

--select top 100 * from [final_master_AD_2]

--select * from Inv_and_metrics_table where line=1457125 order by fiscal_year ,fiscal_week
...
select * from Inv_and_metrics_table where line=1619912
...
,case when max(sum(first_wk_inv)) over(partition by cat_key)=0 then 0 else sum(sum(NET_SALES_UNITS)) over(partition by cat_key order by Age_in_weeks rows unbounded preceding)*1.0/max(sum(first_wk_inv)) over(partition by cat_key) end as CST
 ...
 select * from PLC_AD_Cat_level

--## Adding PLC results to PLC AD

drop table if exists PLC_Stages_line;
select  a.* 
