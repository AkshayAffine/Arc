...
,LEAD(gross_sales_units-net_sales_units,2) 
	over(partition by line order by fiscal_year,fiscal_week) as Returns_post_2wk
from RR_error_AD_All_wks 
)a
...
--Calculating RR using MA method and shifting weeks by 2
drop table if exists RR_error_AD_All_wks_MA_post2wks;
SELECT *
,case when CAST(b.GROSS_SALES as int)=0 then 0 else (CAST(b.Returns_post_2wk as int)*1.0/CAST(b.GROSS_SALES as int)) end AS Wkly_RR
,case when sum(GROSS_SALES) over(partition by dept_desc ,cat_key ,cat_desc  order by fiscal_year ,fiscal_week
                      rows between 13 preceding and 1 preceding) =0 then 0 
else sum(Returns_post_2wk) over(partition by dept_desc ,cat_key ,cat_desc  order by fiscal_year ,fiscal_week
                      rows between 13 preceding and 1 preceding) *1.0/
sum(GROSS_SALES) over(partition by dept_desc ,cat_key ,cat_desc  order by fiscal_year ,fiscal_week
                      rows between 13 preceding and 1 preceding) 
end as ma_13_wk_RR
...
INTO RR_error_AD_All_wks_MA_post2wks
FROM
(
SELECT Dept_desc ,cat_key ,cat_desc  ,FISCAL_YEAR ,FISCAL_WEEK
--,min(cast(launch_date as int)) as launch_date
...
--Checks
select * 
from RR_error_AD_All_wks_MA_post2wks
where  cat_key=14422

select * from RR_error_AD_All_wks_MA_post2wks
where FISCAL_WEEK=39 and FISCAL_YEAR=2014



--## Product level
--Calculating RR using MA method and shifting weeks by 2
drop table if exists RR_error_AD_All_wks_MA_post2wks_line;
SELECT *
,case when CAST(b.GROSS_SALES as int)=0 then 0 else (CAST(b.Returns_post_2wk as int)*1.0/CAST(b.GROSS_SALES as int)) end AS Wkly_RR
,case when sum(GROSS_SALES) over(partition by dept_desc ,cat_key ,cat_desc  order by fiscal_year ,fiscal_week
                      rows between 13 preceding and 1 preceding) =0 then 0 
...
INTO RR_error_AD_All_wks_MA_post2wks_line
FROM
(
SELECT Dept_desc ,cat_key ,cat_desc  ,line ,FISCAL_YEAR ,FISCAL_WEEK
--,min(cast(launch_date as int)) as launch_date
,sum(gross_sales_units) as GROSS_SALES
,(sum(gross_sales_units)-sum(net_sales_units)) AS RETURN_UNITS
,sum(Returns_post_2wk) as Returns_post_2wk
...
--where FISCAL_YEAR =2016 
GROUP BY Dept_desc ,cat_key ,cat_desc ,line ,FISCAL_YEAR ,FISCAL_WEEK
)b
join 
(select cat_key as key_cat ,min(cast(case when coalesce(launch_date,999999)=0 then 999999 else launch_date end as int)) as min_LD
from Final_master_AD_2 group by Cat_key
...
--## 6wk MA
--Calculating RR using MA method and shifting weeks by 2
drop table if exists RR_error_AD_All_wks_6WK_MA_post2wks;
SELECT *
,case when CAST(b.GROSS_SALES as int)=0 then 0 else (CAST(b.Returns_post_2wk as int)*1.0/CAST(b.GROSS_SALES as int)) end AS Wkly_RR
,case when sum(GROSS_SALES) over(partition by dept_desc ,cat_key ,cat_desc  order by fiscal_year ,fiscal_week
                      rows between 6 preceding and 1 preceding) =0 then 0 
else sum(Returns_post_2wk) over(partition by dept_desc ,cat_key ,cat_desc  order by fiscal_year ,fiscal_week
...
,case when sum(GROSS_SALES) over(partition by dept_desc ,cat_key ,cat_desc  order by fiscal_year ,fiscal_week
                      rows between 5 preceding and current row) =0 then 0 
else sum(Returns_post_2wk) over(partition by dept_desc ,cat_key ,cat_desc  order by fiscal_year ,fiscal_week
                      rows between 5 preceding and current row) *1.0 /
sum(GROSS_SALES) over(partition by dept_desc ,cat_key ,cat_desc  order by fiscal_year ,fiscal_week
                      rows between 5 preceding and current row) 
end as pred_RR
INTO RR_error_AD_All_wks_6WK_MA_post2wks
FROM
(
SELECT Dept_desc ,cat_key ,cat_desc  ,FISCAL_YEAR ,FISCAL_WEEK
--,min(cast(launch_date as int)) as launch_date
